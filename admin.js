...
// Проверка, является ли пользователь админом (с проверкой в БД)
async function isUserAdminRealTime(username) {
    if (username === ADMIN_NAME) return true;
...
// Проверка, является ли текущий пользователь админом (с проверкой в БД)
async function isCurrentUserAdmin() {
    const user = getCurrentUser();
    if (!user) return false;
    return await isUserAdminRealTime(user.username);
}

// Проверка, является ли текущий пользователь главным админом
function isMainAdmin() {
    const user = getCurrentUser();
    return user && user.username === ADMIN_NAME;
}

// Создание/обновление кнопки админ-панели
...
async function updateAdminButton() {
    ...
    if (isAdmin && !existingBtn) {
        ...
        adminButton.style.fontSize = '22px';
        adminButton.style.background = 'rgba(255,215,0,0.3)';
        ...
        adminButton.style.transition = 'all 0.2s';
        adminButton.title = 'Админ панель';
        
        const profileBtn = document.getElementById('profileBtn');
        if (profileBtn) {
            sidebarHeader.insertBefore(adminButton, profileBtn);
        } else {
            sidebarHeader.appendChild(adminButton);
        }
        ...
        // Удаляем кнопку
        console.log("❌ Удаляем кнопку админки - права отозваны");
        existingBtn.remove();
        return false;
    }
...
// Периодическая проверка прав админа
function startAdminStatusCheck() {
    ...
    adminCheckInterval = setInterval(async () => {
        const user = getCurrentUser();
        if (user) {
            const wasAdmin = !!document.getElementById('adminPanelBtn');
            const isAdmin = await isCurrentUserAdmin();
            
            if (wasAdmin && !isAdmin) {
                console.log("⚠️ Права админа отозваны, удаляем кнопку");
                const btn = document.getElementById('adminPanelBtn');
                if (btn) btn.remove();
                if (window.showNotification) {
                    window.showNotification("Ваши права администратора отозваны", true);
                }
            } else if (!wasAdmin && isAdmin) {
                console.log("✅ Получены права админа, создаём кнопку");
                await updateAdminButton();
                if (window.showNotification) {
                    window.showNotification("Вам выданы права администратора!");
                }
            }
        }
...
async function showAdminPanel() {
    // Проверяем актуальные права перед открытием
    const isAdmin = await isCurrentUserAdmin();
    if (!isAdmin && !isMainAdmin()) {
        if (window.showNotification) {
            window.showNotification("У вас нет прав администратора", true);
        }
        const btn = document.getElementById('adminPanelBtn');
        if (btn) btn.remove();
        return;
    ...
    try {
        ...
        document.querySelectorAll('.admin-make-admin').forEach(btn => {
            btn.onclick = async () => {
                ...
                // Обновляем кнопку админки у текущего пользователя
                await updateAdminButton();
            };
        });
        ...
        document.querySelectorAll('.admin-remove-admin').forEach(btn => {
            btn.onclick = async () => {
                ...
                // Обновляем кнопку админки у текущего пользователя
                await updateAdminButton();
                // Если права отозваны у текущего пользователя, закрываем модалку
                const currentUser = getCurrentUser();
                if (currentUser && btn.dataset.user === currentUser.username) {
...
async function setUserBlocked(username, isBlocked) {
    ...
    const isAdmin = await isUserAdminRealTime(currentUser.username);
    if (!isAdmin && currentUser.username !== ADMIN_NAME) {
        if (window.showNotification) window.showNotification("У вас нет прав для блокировки!", true);
        return;
    }
