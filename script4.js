...
async function loadChats() {
    ...
    for (const chatId in userChats) {
        if (chatId !== 'general' && !chatId.startsWith('group_')) {
            ...
            if (userCheck.exists()) {
                const userAvatar = await getUserAvatar(chatId);
                const hasCarrot = await hasCarrotAvatar(chatId);
...
function renderChats(chats = null) {
    ...
    for (const chat of list) {
        ...
        if (chat.type === 'private') {
            avatarHtml = `<div class="chat-avatar">${chat.avatar || '👤'}</div>`;
            if (chat.hasCarrot) {
                nameClass = 'rainbow-name';
            }
        } else {
            avatarHtml = `<div class="chat-avatar">${chat.avatar || chat.icon}</div>`;
        }
        ...
        html += `<div class="chat-item ${currentChat.id === chat.id ? 'active' : ''}" 
            ...
            ${avatarHtml}
            <div class="chat-info">
                <div class="chat-name ${nameClass}">${escapeHtml(chat.name)}</div>
...
function renderMessages(messages) {
    ...
    // Асинхронно обновляем аватарки и радужные имена для чужих сообщений
    document.querySelectorAll('.message.other').forEach(async (el) => {
        const nameSpan = el.querySelector('.message-info span');
        const senderName = nameSpan ? nameSpan.textContent : '';
        if (senderName && senderName !== currentUser.username) {
            const avatar = await getUserAvatar(senderName);
            const hasCarrot = await hasCarrotAvatar(senderName);
            const avatarDiv = el.querySelector('.message-avatar');
            if (avatarDiv) avatarDiv.textContent = avatar;
            if (hasCarrot && nameSpan) {
...
function createMessageHTML(msg) {
    ...
    return `<div class="message ${isOwn ? 'own' : 'other'}" data-mid="${msg.id}" data-sender="${escapeHtml(msg.sender)}">
        <div class="message-avatar">${isOwn ? '👤' : '👤'}</div>
        <div class="message-content">
            <div class="message-info">
                <span class="${nameColorClass}">${escapeHtml(msg.sender)}</span> • ${time}
...
function appendMessage(msg, msgId) {
    // ============ УВЕДОМЛЕНИЕ ДЛЯ ТЕЛЕФОНА ============
    // Проверяем: сообщение не от меня И чат НЕ общий
    if (currentUser && msg.sender !== currentUser.username && currentChat.type !== 'general') {
        var title = "📩 " + msg.sender;
        var body = msg.text ? msg.text.substring(0, 100) : "Голосовое сообщение";
        
        // Показываем уведомление только если окно не активно (пользователь не в чате)
    ...
    const html = `<div class="message ${isOwn ? 'own' : 'other'}" data-mid="${msgId}" data-sender="${escapeHtml(msg.sender)}">
        <div class="message-avatar">${isOwn ? '👤' : '👤'}</div>
        <div class="message-content">
    ...
    // Асинхронно обновляем аватарку для чужих сообщений
    if (!isOwn) {
        const senderName = msg.sender;
        (async () => {
            const avatar = await getUserAvatar(senderName);
            const hasCarrot = await hasCarrotAvatar(senderName);
            const avatarDiv = newEl.querySelector('.message-avatar');
            const nameSpan = newEl.querySelector('.message-info span');
            if (avatarDiv) avatarDiv.textContent = avatar;
            if (hasCarrot && nameSpan) {
                nameSpan.classList.add('rainbow-name');
            }
...
console.log("✅ script4.js загружен (с уведомлениями)");