...
function appendMessage(msg, msgId) {
    ...
    // и что текущий чат не общий
    if (msg.sender !== currentUser.username && currentChat.type !== 'general') {
        const title = `📩 Новое сообщение от ${msg.sender}`;
        const body = msg.text ? msg.text.substring(0, 100) : "Голосовое сообщение";
        const tag = currentChat.id;
        
        // Отправляем уведомление (если окно не активно)
