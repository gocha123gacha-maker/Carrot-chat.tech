...
document.getElementById('confirmSendCoins').addEventListener('click', async () => {
    ...
    if (!userExists.exists()) { err.textContent = 'Пользователь не найден'; return; }
...
document.getElementById('confirmAddMember').addEventListener('click', async () => {
    ...
    if (!userExists.exists()) { document.getElementById('addMemberError').textContent = 'Пользователь не найден'; return; }
...
// ============ УДАЛЕНИЕ ЧУЖИХ СООБЩЕНИЙ ДЛЯ ПРЕМИУМ ============
...
async function checkDailyBonus() {
    ...
    const lastBonusRef = database.ref(`lastBonus/${currentUser.username}`);
    const snap = await lastBonusRef.get();
