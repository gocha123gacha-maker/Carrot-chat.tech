...
// ============ ПРОВЕРКА ПРЕМИУМА У ЛЮБОГО ПОЛЬЗОВАТЕЛЯ ============
async function hasPremiumByUsername(username) {
    try {
        const snap = await database.ref(`premium/${username}`).get();
...
window.hasPremiumByUsername = hasPremiumByUsername;
