...
function loadMessages(chatId) {
    ...
    // Вешаем слушатель на новые сообщения
    ...
    // Также слушаем изменения (редактирование)
...
function displayMessage(messageId, msg) {
    const container = document.getElementById('messages-container');
    ...
    if (currentChatId === 'general') {
        // В общем чате всегда показываем отправителя
        div.innerHTML = `<div class="message-sender">${msg.sender}</div>${msg.text}`;
        if (msg.edited) {
            div.innerHTML += ' <span style="font-size: 0.7rem; opacity: 0.7;">(ред.)</span>';
        }
    } else {
        // В личном чате
        if (msg.sender !== currentUser) {
            // Сообщение от собеседника - показываем имя
            div.innerHTML = `<div class="message-sender">${msg.sender}</div>${msg.text}`;
            if (msg.edited) {
                div.innerHTML += ' <span style="font-size: 0.7rem; opacity: 0.7;">(ред.)</span>';
            }
        } else {
            // Своё сообщение - только текст
    ...
    // Добавляем обработчик двойного клика только для своих сообщений
    if (msg.sender === currentUser) {
    ...
    // Скролл вниз для нового сообщения
    container.scrollTop = container.scrollHeight;
}
...
function updateMessage(messageId, msg) {
    ...
    if (currentChatId === 'general') {
        messageElement.innerHTML = `<div class="message-sender">${msg.sender}</div>${msg.text}`;
        if (msg.edited) {
            messageElement.innerHTML += ' <span style="font-size: 0.7rem; opacity: 0.7;">(ред.)</span>';
        }
    } else {
        if (msg.sender !== currentUser) {
            messageElement.innerHTML = `<div class="message-sender">${msg.sender}</div>${msg.text}`;
            ...
            if (msg.edited) {
                messageElement.textContent += ' (ред.)';
            }
        }
    }
}
...
function sendMessage() {
    ...
    // Проверяем, не заблокирован ли собеседник (для личных чатов)
    if (currentChatId !== 'general') {
        database.ref('users/' + currentChatId).once('value', snapshot => {
            const userData = snapshot.val();
            if (userData && userData.blocked) {
                alert('Этот пользователь заблокирован администратором');
                return;
            }
            proceedSendMessage(text);
        });
    } else {
        proceedSendMessage(text);
    }
...
function proceedSendMessage(text) {
    ...
    database.ref('messages/' + currentChatId + '/' + messageId).set(message)
        .then(() => {
            ...
            // Сообщение отобразится через слушатель child_added
        })
        .catch(error => {
