// Подключаем Firebase глобально (нужно добавить script в HTML)
    ...
    for (const [chatId, v] of Object.entries(userChats)) {
        if (chatId !== 'general' && !chatId.startsWith('group_')) {
            ...
            if (userCheck.exists()) {
                ...
                await database.ref(`users/${currentUser.username}/chats/${chatId}`).remove();
            }
        }
    }
...
function showEditModal(msgId, txt) {
    ...
    document.getElementById('saveEdit').onclick = async () => {
        ...
        await database.ref(`${path}/${msgId}`).update({ text: newText, edited: true, editedAt: Date.now() });
        close();
...
function setupRealtimeMessages(path) {
    ...
    messagesListener = database.ref(path).on('child_added', async (data) => {
        const msg = data.val();
    ...
    database.ref(path).on('child_changed', async (data) => {
        const msg = data.val();
...
document.getElementById('sendBtn').addEventListener('click', async () => {
    ...
    await database.ref(path).push({ text, sender: currentUser.username, timestamp: Date.now() });
    input.value = '';
});
...
async function cleanupOldVoiceMessages() {
    ...
    const chatsSnap = await database.ref(`users/${currentUser.username}/chats`).get();
    ...
    for (const chatId of Object.keys(chats)) {
        ...
        const messagesSnap = await database.ref(path).get();
        ...
        if (messages) {
            for (const [msgId, msg] of Object.entries(messages)) {
                if (msg.type === 'voice' && msg.timestamp && msg.timestamp < fourDaysAgo) {
                    await database.ref(`${path}/${msgId}`).remove();
                    console.log(`Удалено голосовое сообщение ${msgId}`);
                }
            }
        }
    }
...
async function loadPremiumPrice() {
    ...
    if (snap.exists()) {
        ...
        await database.ref('premiumPrice').set(PREMIUM_PRICE);
    }
}
