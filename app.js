...
async function displayMessages(messages) {
    ...
    for (const msg of messages) {
        ...
        html += `<div class="message ${msg.sender === currentUser.username ? 'own' : 'other'}" data-msg-id="${msg.id}" data-msg-text="${escapeHtml(msg.text||'')}">
            <div class="message-avatar">${msg.sender === currentUser.username ? '👤' : avatar}</div>
            <div class="message-content">
                <div class="message-info">
                    <span class="${nameColor}">${escapeHtml(msg.sender)}</span> • ${new Date(msg.timestamp).toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'})}
    ...
    document.querySelectorAll('.message.own').forEach(el => {
        el.addEventListener('dblclick', () => {
            const id = el.dataset.msgId, txt = el.dataset.msgText;
...
function setupRealtimeMessages(path) {
    ...
    messagesListener = onChildAdded(msgRef, async (data) => {
        ...
        div.innerHTML = `<div class="message-avatar">${msg.sender === currentUser.username ? '👤' : avatar}</div>
            <div class="message-content">
                <div class="message-info">
                    <span class="${nameColor}">${escapeHtml(msg.sender)}</span> • ${new Date(msg.timestamp).toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'})}
                    ${msg.edited ? ' <span style="font-size:9px;">(изменено)</span>' : ''}
        ...
        if (msg.sender === currentUser.username && msg.text) {
            div.addEventListener('dblclick', () => showEditModal(data.key, msg.text));
        }
    });
    ...
    onChildChanged(msgRef, async (data) => {
        ...
        if (el) {
            const infoSpan = el.querySelector('.message-info span');
            if (infoSpan) infoSpan.innerHTML = escapeHtml(msg.sender);
