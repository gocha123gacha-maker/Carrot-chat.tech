...
// Функция для обновления статуса админки
window.updateAdminRights = async () => {
    if (window.adminFunctions && window.adminFunctions.updateAdminButton) {
        await window.adminFunctions.updateAdminButton();
    }
};

// Следим за изменениями currentUser
let lastUsername = currentUser?.username;
setInterval(() => {
    if (currentUser && currentUser.username !== lastUsername) {
        lastUsername = currentUser?.username;
        if (window.updateAdminRights) window.updateAdminRights();
    }
}, 1000);
}